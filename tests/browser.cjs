...
(async () => {
  ...
  try {
    ...
    async function assertStartupDefaults() {
      ...
      assert.ok(await page.locator('#instrument-stage').evaluate(el => el.classList.contains('with-gourd')));
      assert.ok(await page.locator('#soundboard').evaluate(el => el.classList.contains('hide-labels')));
      assert.equal(await page.locator('#voice').inputValue(), 'reference');
      assert.equal(await page.locator('#buzz').inputValue(), '100');
      assert.equal(await page.locator('#sustain').inputValue(), '1');
    ...
    assert.equal(await page.locator('.touch-pad').count(), 39);
    ...
    await page.locator('#buzz').fill('25');
    await page.locator('#sustain').fill('1.8');
    ...
    assert.equal(await page.locator('#tuning-root').inputValue(), 'f');
    assert.equal(await page.locator('#voice').inputValue(), 'nyunga');
    const nyungaSignal = await page.evaluate(async () => {
      const context = new OfflineAudioContext(1, 44100, 44100);
      const bank = MbiraAudio.referenceBank(context, 'nyunga');
      const old = MbiraAudio.referenceBank(context, 'reference');
      MbiraAudio.voice(context, context.destination, 74, 0, { voice: 'nyunga', buzz: 1, sustain: 1 });
      const data = (await context.startRendering()).getChannelData(0);
      return { distinct: bank !== old && bank[0].tone !== old[0].tone, count: bank.length, peak: data.reduce((p, x) => Math.max(p, Math.abs(x)), 0) };
    });
    assert.ok(nyungaSignal.distinct);
    assert.equal(nyungaSignal.count, 6);
    assert.ok(nyungaSignal.peak > .05 && nyungaSignal.peak < 1);

    const expected = [74,81,72,79,70,77,62,82,58,77,65,79,67,81,69];
    assert.deepEqual(await page.evaluate(() => MbiraMusic.nyungaKeys.map(currentPitch)), expected);
    await page.locator('.nyunga-key').nth(7).click();
    await page.waitForFunction(() => document.getElementById('last-note').textContent.includes('932 Hz'));
    await page.locator('#tuning-root').selectOption('bb');
    assert.deepEqual(await page.evaluate(() => MbiraMusic.nyungaKeys.map(currentPitch)), expected.map(n => n + 5));
    await page.locator('#record').click();
    await page.keyboard.press('q');
    await page.waitForTimeout(200);
    await page.locator('#record').click();
    assert.equal(await page.evaluate(() => session.tracks[0].events[0].midi), 79);
    assert.equal(await page.evaluate(() => session.tracks[0].sound.voice), 'nyunga');
    assert.ok(await page.evaluate(() => MbiraMusic.validSession(session)));
    await page.reload();
    assert.equal(await page.evaluate(() => session.tracks[0].events[0].id), 'nyunga-1');
    assert.equal(await page.evaluate(() => session.tracks[0].sound.voice), 'nyunga');
    const nyungaDownloadPromise = page.waitForEvent('download');
    await page.locator('#download').click();
    await (await nyungaDownloadPromise).saveAs(path.join(output, 'nyunga-mix.wav'));
    await page.locator('#instrument-type').selectOption('nyunga');
    assert.equal(await page.locator('#tuning-root').inputValue(), 'bb');
    assert.ok(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth));
